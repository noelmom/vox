...
def run_verifier(tmp_path: Path, payload: str, *, missing_current_symlink: bool = False) -> subprocess.CompletedProcess[str]:
    package = tmp_path / "Vox-1.2.3.pkg"
    package.write_bytes(b"fixture package")
    bin_dir = tmp_path / "bin"
    bin_dir.mkdir(exist_ok=True)
    for name, body in {
        "pkgutil": f"#!/bin/sh\nif [ \"$1\" = \"--payload-files\" ]; then printf '%s\\n' '{payload}'; exit 0; fi\nif [ \"$1\" = \"--expand-full\" ]; then root=\"$3/Payload/Applications/Vox/VoxHelper.app/Contents\"; mkdir -p \"$root/Frameworks/Sparkle.framework/Versions/A\" \"$root/MacOS\"; touch \"$root/Frameworks/Sparkle.framework/Versions/A/Sparkle\" \"$root/MacOS/VoxHelper\"; [ \"${{VOX_TEST_NO_CURRENT:-}}\" = 1 ] || ln -s A \"$root/Frameworks/Sparkle.framework/Versions/Current\"; exit 0; fi\nexit 0\n",
        "spctl": "#!/bin/sh\nexit 0\n",
        ...
        path.chmod(0o755)
    environment = {**os.environ, "PATH": f"{bin_dir}:{os.environ['PATH']}", "VOX_TEST_NO_CURRENT": "1" if missing_current_symlink else ""}

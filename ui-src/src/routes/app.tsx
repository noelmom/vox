...
function GenerationStatusBar() {
  ...
  return <div role="status" aria-live="polite" className="flex min-h-12 items-center gap-3 border-b border-blue-100 bg-blue-50/80 px-4 py-2 text-sm md:px-8"><span className="h-2.5 w-2.5 animate-pulse rounded-full bg-blue-600 motion-reduce:animate-none" /><strong>{status}</strong><span className="min-w-0 flex-1 truncate text-foreground/55">{state.phase === "polling" ? `Job ${state.requestId.slice(0, 8)}` : "Sending your script to Vox"}</span>{state.phase === "polling" && <button type="button" disabled={state.status === "cancelling"} onClick={async () => { const result = await cancelJob(state.requestId); if (result.status !== "cancelled") setGenerationState({ ...state, status: "cancelling" }); }} className="min-h-10 rounded-lg border border-border bg-white px-3 text-xs font-semibold disabled:opacity-50">{state.status === "cancelling" ? "Stopping…" : "Stop"}</button>}</div>;
}

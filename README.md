...
| `VOX_MPS_MEMORY_FRACTION` | `0.9` | Apple GPU memory allocator cap used before loading the model. `0.9` is the stable default; `1.0` allows full recommended MPS memory. This is not a GPU compute throttle and does not make generation deterministic. Changes require restarting the local server. |
